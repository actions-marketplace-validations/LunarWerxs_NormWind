// Vendored from eslint-plugin-tailwindcss v3.18.3's lib/config/groups.js.
//
// eslint-plugin-tailwindcss 4.x rewrote shorthand/group classification to
// query a live Tailwind v4 design system through a synckit worker
// (lib/worker/tailwind-api.mjs, backed by the `tailwind-api-utils` package)
// instead of shipping a static regex table, and it no longer exports one
// publicly or internally. lib/shorthand-families.mjs needs exactly the
// static table (the hierarchy of utility `members` grouped by CSS property,
// used to detect which shorthand a set of longhands can collapse into), so
// this is a one-time port of the last version that had it, not a bump.
//
// Copyright (c) Francois Massart, MIT licensed. Full text in
// THIRD-PARTY-NOTICES.md at the repo root. Upstream source:
// https://github.com/francoismassart/eslint-plugin-tailwindcss/blob/v3.18.3/lib/config/groups.js
//
// This is a data snapshot, not a live dependency: it will not receive
// upstream fixes. If eslint-plugin-tailwindcss ever re-exposes an
// equivalent table, prefer importing it again over maintaining this copy.
